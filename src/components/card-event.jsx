export default function CardEvent({ events, onDeleteEvent }) {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 p-6">
            {events.map((event) => (
                <div key={event.idNumber} className="group bg-gradient-to-br from-white via-gray-50 to-gray-100 rounded-2xl shadow-lg hover:shadow-2xl overflow-hidden hover:-translate-y-2 transition-all duration-500 mx-5 my-5 relative border border-gray-200">
                    {/* Delete Button */}
                    <button
                    ...
                    <div className="p-6 bg-white">
                        <h3 className="text-xl font-bold text-gray-900 mb-3 group-hover:text-blue-600 transition-colors duration-200 overflow-hidden text-ellipsis whitespace-nowrap">
                            {event.name}
                        </h3>
                        ...
                        <div className="flex items-center text-gray-600 mb-3">
                            ...
                            <div>
                                <p className="font-medium text-sm">{event.day}, {event.date}</p>
                                <p className="text-xs text-gray-500">Pukul {event.time}</p>
                            </div>
                        </div>
